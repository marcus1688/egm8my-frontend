<template>
  <ClientOnly>
    <div class="bg-slate-50 min-h-screen">
      ...
      <section class="py-12 relative max-lg:p-4">
        <div class="container mx-auto px-4 mb-12 max-lg:hidden">
          <div class="grid grid-cols-2 md:grid-cols-6 gap-4">
            <div
              v-for="level in settingsData.vipLevels"
              :key="level.id"
              class="bg-gradient-to-b from-white to-gray-100 rounded-xl shadow-md overflow-hidden transform transition-all lg:hover:scale-105 lg:hover:shadow-lg"
            >
              <div
              ...
              <div class="p-4 text-center">
                <h3
                  ...
                  :class="{
                    ...
                    'text-gray-500': level.name === 'Silver',
                    'text-[#daa520]': level.name === 'Gold',
                    'text-[#3d898f]': level.name === 'Platinum',
                    'text-gray-600': level.name === 'Diamond',
                  }"
                >
                  {{ level.name }}
                </h3>
        ...
        <div class="container mx-auto px-4 mb-12 max-lg:px-2 max-lg:mb-6">
          <div
            class="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200 relative max-lg:rounded-lg max-lg:shadow-md"
          >
            <!-- Glowing corner effects -->
            <div
              class="absolute -top-4 -left-4 w-8 h-8 bg-blue-500 rounded-full blur-xl opacity-20 max-lg:w-6 max-lg:h-6"
            ></div>
            <div
              class="absolute -bottom-4 -right-4 w-8 h-8 bg-purple-500 rounded-full blur-xl opacity-20 max-lg:w-6 max-lg:h-6"
            ></div>
              ...
              <table class="w-full min-w-[900px] max-lg:min-w-[750px]">
                ...
                <tbody>
                  <template
                    ...
                    :key="row.id"
                  >
                    <tr v-if="isFirstInGroup(rowIndex)" class="bg-blue-50/40">
                      <td
                        colspan="7"
                        class="py-2 px-6 font-medium text-blue-800 max-lg:py-1.5 max-lg:px-3 max-lg:text-sm"
                      >
                        {{ getSectionTitle(row.name) }}
                      </td>
                    </tr>
                    <tr class="lg:hover:bg-gray-50 transition-colors">
                      <td
                        class="py-3 px-6 border-t border-gray-100 font-medium max-lg:py-2 max-lg:px-3 max-lg:text-sm"
                      >
                      ...
                      <td
                        ...
                        class="py-3 px-4 text-center border-t border-gray-100 max-lg:py-2 max-lg:px-2 max-lg:text-xs"
                      >
                        <span
                          v-if="
                            level.benefits &&
            ...
            <!-- Mobile hint -->
            <div
              class="text-center text-xs text-gray-500 p-2 border-t border-gray-100 bg-gray-50 md:hidden max-lg:py-3"
            >
              <p>
        ...
        <div>
          <div
            class="bg-white rounded-xl shadow-lg !py-6 container mx-auto !px-4 border border-gray-200 relative max-lg:rounded-lg max-lg:shadow-md max-lg:!py-4 max-lg:!px-3"
          >
            <div
              class="absolute top-0 right-0 w-20 h-20 overflow-hidden max-lg:w-12 max-lg:h-12"
            >
              <div
                class="absolute top-0 right-0 w-4 h-4 bg-blue-600 transform rotate-45 translate-x-1/2 -translate-y-1/2 max-lg:w-3 max-lg:h-3"
              ></div>
            </div>

            <h3
              class="text-lg font-bold text-gray-800 mb-4 flex items-center max-lg:text-base max-lg:mb-3"
            >
            ...
            <div
              ...
              class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3 text-gray-600 text-sm max-lg:gap-x-4 max-lg:gap-y-2 max-lg:text-xs"
            >
              <ul class="space-y-2 list-none max-lg:space-y-1.5">
                ...
                >
                  <span
                    class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-blue-100 text-blue-600 text-xs font-bold mr-2 flex-shrink-0 max-lg:w-4 max-lg:h-4 max-lg:text-[10px] max-lg:mr-1.5"
                    >{{ index + 1 }}</span
                  >
                  <span>{{ item }}</span>
                </li>
